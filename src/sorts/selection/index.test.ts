...
describe('选择排序', () => {
  ...
  it('传递 modifyOriginal 为 false，不应修改原数组', () => {
    const input = [3, 2, 1];
    const expected = [1, 2, 3];
    const result = selection(input, false);
  ...
  it('使用自定义 compareFunction 进行逆序排序', () => {
    ...
    const result = selection(input, true, customCompare);
    expect(result).toEqual(expected);
  });
  ...
  it('传递自定义的比较函数，用于混合类型数组', () => {
    ...
    const result = selection(input, true);
    expect(result).toEqual(expected);
  });
  ...
  it('反转排序', () => {
    ...
    const result = selection(input, true, undefined, true);
    expect(result).toEqual(expected);
  });
});
