...
describe('煎饼排序', () => {
  ...
  it('传递 modifyOriginal 为 false，不应修改原数组', () => {
    ...
    const result = pancake(input, false);
  ...
  it('使用自定义 compareFunction 进行逆序排序', () => {
    ...
    const result = pancake(input, true, customCompare);
    expect(result).toEqual(expected);
  });
  ...
  it('传递自定义的比较函数，用于混合类型数组', () => {
    ...
    const result = pancake(input, true);
    expect(result).toEqual(expected);
  });
  ...
  it('使用不同的 runSize 进行排序', () => {
    ...
    const resultWithSmallRun = pancake(input, true);
    const resultWithLargeRun = pancake(input, true);
  ...
  it('反转排序', () => {
    ...
    const result = pancake(input, true, undefined, true);
    expect(result).toEqual(expected);
  });
});
