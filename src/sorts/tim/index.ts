...
/**
 * 定序排序（Tim Sort）
 * @description 定序排序是一种混合排序算法，结合了插入排序和归并排序的优点，对部分有序的数据集表现出色
 * @usageScenario 适用于各种大小的数据集，特别是部分有序数据
 * @timeComplexity 平均情况 O(n log n)，最坏情况 O(n log n)，最好情况 O(n)
 ...
 * @param {number} [runSize = 32] 分块大小，用于决定何时使用插入排序
 * @returns {T[]} 返回排序后的数组
 */
