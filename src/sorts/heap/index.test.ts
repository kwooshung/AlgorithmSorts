...
describe('堆排序', () => {
  ...
  it('使用自定义 compareFunction 进行逆序排序', () => {
    ...
    const result = heap(input, true, customCompare);
    expect(result).toEqual(expected);
  });
  ...
  it('反转排序', () => {
    ...
    const result = heap(input, true, undefined, true);
    expect(result).toEqual(expected);
  });
});
