...
describe('希尔排序', () => {
  ...
  it('传递 modifyOriginal 为 false，不应修改原数组', () => {
    const input = [3, 2, 1];
    const expected = [1, 2, 3];
    const result = shell(input, false);
  ...
  it('使用自定义 compareFunction 进行逆序排序', () => {
    ...
    const result = shell(input, true, customCompare);
    expect(result).toEqual(expected);
  });
  ...
  it('反转排序', () => {
    ...
    const result = shell(input, true, undefined, true);
    expect(result).toEqual(expected);
  });
});
